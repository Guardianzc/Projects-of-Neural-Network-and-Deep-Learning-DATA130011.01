'''
@Author: your name
@Date: 2020-06-10 17:02:40
@LastEditTime: 2020-06-19 12:05:02
@LastEditors: Please set LastEditors
@Description: In User Settings Edit
@FilePath: /final/EAST/detect.py
'''
import torch
...
def resize_img(img,length = 800):
	'''resize image to be divisible by 32
	'''
	w, h = img.size
	r = min(length/w,length/h)
...
def detect(img_file,img, model, device):
	...
	with torch.no_grad():
		img_on_gpu = load_pil(img).to(device)
		try:
			score, geo = model(img_on_gpu)
			boxes = get_boxes(score.squeeze(0).cpu().numpy(), geo.squeeze(0).cpu().numpy())
			return adjust_ratio(boxes, ratio_w, ratio_h)
		except RuntimeError as exception:
			if "out of memory" in str(exception):
				print(img_on_gpu.shape)
				print(img_file)
			else:
				raise exception
...
def detect_dataset(model, device, test_img_path, submit_path):
	...
	for i, img_file in enumerate(img_files):
		...
		boxes = detect(img_file,Image.open(img_file).convert('RGB'), model, device)
		seq = []
